...
class EscapeMaze_Solution {
    private final static int MAX_N = 1001;//1부터 인덱스 사용하기 위해
    private final static int INF = Integer.MAX_VALUE;
    
    int[][] Graph = new int[MAX_N][MAX_N];
    int dijksstra(int n,int src,int dst,int[] traps){//n:정점 갯수, src:출발점, dst:도착점, traps : 함정 노드들
            ...
            for(int i=0;i<traps.length;++i){
                ...
                if((state & bit)!= 0) {//0이 아니라는 건 그 비트가 켜져있다는 뜻.
                    ...
                    if(traps[i] ==u) {//비트를 끄는 방법은 비트 반전시킨다음 &연산하면 된다.
                        state = state & ~bit;//~bit는ㄴ 해당 비트만 0이다. 따라서 현재 상태 state와 &연산하면 해당 비트만 0이 된다.
                    } else{//아니라면 반전 시킬 필요없음
                        trapped.put(traps[i],true);
                    }
                } else{//함정 발동X 상태 : 함정 발동 비트 ON 한다!
                    if(traps[i] == u){
                        state = state | bit;//함정에 해당하는 비트 켠다!
    ...
    public int solution(int n, int start, int end, int[][] roads, int[] traps) {
        ...
        return dijksstra(n,start,end,traps);
    }
}