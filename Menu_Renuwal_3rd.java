...
class Menu_Renuwal_3rd {
    ...
    static void makeMap(int index,int select,String str,String order,int target){
        ...
        if(index == target){
            ...
            m = Math.max(m,map.get(tmp));//course[i]길이의 최빈값 갖는 문자열을 찾아서 그 빈도수를 저장!
            return;
        }
    ...
    public String[] solution(String[] orders, int[] course) {
        PriorityQueue<String> pq = new PriorityQueue<>();//문자열들을 사전순으로 저장하기 위해!
        for(int i=0;i<course.length;i++){
            ...
            for(String s : map.keySet()){
                //if(map.get(s) == m && s.length()>=2){//단품메뉴 2개 이상이여야함.=>문자열길이체크 안해도되나???
                if(map.get(s) == m && map.get(s)>1){//빈도수 2이상이여야함.
                    pq.add(s);
                }
            }
        }
