...
class PuzzleGame {
    ...
    public int solution(int[][] game_board, int[][] table) {
        ...
        for(int i=0;i<game_board.length;i++)
            for(int j=0;j<game_board.length;j++)
                game_board = game_board[i][j] == 0 ? 1:0;
        ...
        for(int dir=0;dir<4;dir++){
            table = rotate(table);//배열의 크기를 넣어준다
    ...
    static String bfs(int x,int y,int[][] tmpArr){//1:탐색 가능,0:탐색 X
        //임시 2차원 배열 만들어서 방문하려는 배열 똑같이 복사한다!
        //방문한 노드는 0으로 마킹해서 갈수 없는 노드와 방문한 노드 함께 판단 가능하다!
        Queue<Point> q = new LinkedList<Point>();
        q.add(new Point(x,y));
        tmpArr[x][y] = 0;
        int[] dx = {-1,1,0,0}; int[] dx = {0,0,-1,1};
    ...
    static String makeString(int minX,int maxX,int minY,int maxY,LinkedList<Point> list){
