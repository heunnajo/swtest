...
public class SpecificMagnet_noClassVer {
	static int ans,T,K;
	...
	static void go(int idx,int dir,int rotDir) {
		...
		if(dir==2) {
			if(idx-1>=0 && Magnet[idx-1][2] != Magnet[idx][6]) {go(idx-1,0,-rotDir);}
		...
		//idx-1,idx+1 조건 판단 후 먼저 회전
			...
			if(idx-1>=0 && Magnet[idx-1][2] != Magnet[idx][6]) {//바로 idx-1에 접근하면 안됨.초기실행과 재귀실행 구분?
	...
	public static void main(String[] args) throws Exception{
		...
		for(int t=1;t<=T;t++) {
			...
			for(int i=0;i<K;i++) {
				...
				//System.out.println(i+"이동 전 ");
				//printMagnet();
				go(idx,2,dir);
				//System.out.println(i+"이동 후 ");
				//printMagnet();
			}
